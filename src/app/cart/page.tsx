...
export default function Cart() {
  ...
  const totalAmountBeforeDiscounts = items.reduce((acc, item) => acc + item.price, 0);

  // Calculate total discount amount
  const totalDiscountAmount = items.reduce((acc, item) => acc + (item.price * item.discountPercentage) / 100, 0);
  ...
  return (
    <div className="flex justify-center mt-8">
      <div className="w-3/5 mr-8">
        ...
        {items.length > 0 ? (
          <div>
            ...
            {items.map((item) => (
              <div key={item.id} className="flex items-center border-b border-amber-300 py-4">
                ...
                <div className="w-1/6">1</div> {/* Assuming quantity is 1 for each item */}
                <div className="w-1/4">${item.price - (item.price * item.discountPercentage) / 100}</div>
              </div>
            ))}
          </div>
        ) : (
      ...
      <div className="w-2/5 bg-gray-100 p-4">
        ...
        <div className="mb-2">
          <p>Total Amount Before Discounts: ${totalAmountBeforeDiscounts.toFixed(2)}</p>
          <p>Total Discount Amount: ${totalDiscountAmount.toFixed(2)}</p>
          <p>Total Amount After Discounts: ${totalAmountAfterDiscounts.toFixed(2)}</p>
        </div>
        <button className="bg-amber-600 text-white px-4 py-2 rounded">Checkout</button>
      </div>
    </div>
  );
}
